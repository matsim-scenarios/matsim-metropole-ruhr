...
public class CreateCountsFromBAStData implements MATSimAppCommand {
	...
	@Override
	public Integer call() {
		Map<String, BAStCountStation> stations = readBAStCountStations(stationData, shp, crs);

		// Assigns link ids in the station objects
		matchBAStWithNetwork(network, stations);

		for (Map.Entry<String, Id<Link>> e : counts.getManualMatched().entrySet()) {
	...
	private void match(Network network, Index index, BAStCountStation station) {
		...
		Link matched = NetworkUtils.getNearestLink(network, station.getCoord());
	...
	private void matchBAStWithNetwork(String pathToNetwork, Map<String, BAStCountStation> stations) {
		...
		for (var station : stations.values())
			match(filteredNetwork, index, station);
	}

	private Map<String, BAStCountStation> readBAStCountStations(Path pathToAggregatedData, ShpOptions shp, CrsOptions crs) {
