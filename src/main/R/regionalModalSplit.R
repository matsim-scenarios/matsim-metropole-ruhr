...
completeTripsTable <-readTripsTable("output_trips.csv.gz")
person2home <- read_csv2("person2Home.csv")
...
## rvr Data
rvrModalSplitData <- read_csv2("regionalModalSplitsRVR.csv")
modalSplitsOfModelAndRVR <- left_join(regionalModalSplits, rvrModalSplitData, by = "area")

write_excel_csv2(regionalModalSplits, file = "regionalModalSplitsModelResults.csv")
...
for(i in 1:length(myplots)) {
  png(paste("Plots/ModalSplit", i, ".png"), res=200)
  set.seed(i)
  plot(myplots[[i]]) 
  dev.off()
}
